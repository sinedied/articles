...
- While nothing prevents you using Docker and any IDE to do the same thing, the Remote Development extension provides seamless integration with VS Code to bootstrap and work with dev containers.
...
- "Hey, welcome to team! We're all excited to have you on board!" 🎉
...
After a while spending time reviewing everything you installed, you'll eventually discover common issues usually are:
- The environment setup doc is *outdated* (I bet you didn't see that one coming, right? Isn't that the newcomer's job to update it anyways?).
...
![Screenshot of new folder in VS Code](./assets/config-files.png)
...
VS Code will reload the project's window and start building the container. Depending of your internet connection speed, this may take a while but don't worry, it's only needed the first time.
...
Now that you have a working environment, you'll probably want to customize it for your project's need. If you unfold the `.devcontainer` folder in the explorer, you'll see these 2 files that will allow you to do that:

- `Dockerfile`: this file define your container configuration, and is used to build the container image that will be used for your environment. If you need to install additional tools or configure scripts and environment variables for your dev environment, this is where you should look at.
...
Once your container and VS Code configuration is finished, you only have to push these two files in your repository to make it available for your team. And it's one of the big benefits of this approach! To welcome a new developer on your project, you now only have to clone the project, reload VS Code to use the container config and that's it. Take a coffee (or many) while the container is downloading and installing tools on its first run, and your new developer will have a dev environment ready for work, without any effort.
