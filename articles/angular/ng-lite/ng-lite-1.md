...
> **Note:** In the following section, we'll take some time to understand the base application template and how it can be slimmed down for simpler projects. If you're not interested in that part, you can instead use the command `npx degit sinedied/ng-lite-starter` that will use this pre-made [github template](https://github.com/sinedied/ng-lite-starter) and jump to the [next section](#building-the-todo-list).
...
After the files are generated and the dependencies installed, let's hop into the `ng-lite-todo` folder and start with some cleaning.
...
Now, there's a folder named `assets/` that you can use to put any assets (images, fonts, JSON files...) you want to be copied to the `dist/` folder.It contains an empty `.gitkeep` to get the folder stored in the git repo, as git don't store empty folders. We can simplify the structure a bit:
...
This doesn't change much, but it's more in-line with what you can find in almost all other web frameworks (React, Vue, Svelte...) and it means that you can easily add any new files to be placed at the root of the `dist/` folder without having to edit `angular.json`, something we'll have to do a bit later.
...
We'll move the file directly in `src/` too:

```bash
# Move app.component.ts file to src/
mv src/app/app.component.ts src/
# Delete app/ folder
rm -rf src/app
```
...
Of course, that's a bit tedious and not something you want to do everytime you start a new project! You can use this [github template](https://github.com/sinedied/ng-lite-starter)
